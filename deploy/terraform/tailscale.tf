...
resource "google_compute_subnetwork" "private_staging" {
  # Handle the case where subnet already exists
  lifecycle {
...
resource "google_compute_firewall" "tailscale" {
  # Handle the case where firewall rule already exists
  lifecycle {
    prevent_destroy = true
  }
...
resource "google_service_account" "tailscale" {
  ...
  # Handle the case where service account already exists
  lifecycle {
    prevent_destroy = true
  }
...
resource "google_container_node_pool" "private_staging_nodes" {
  ...
  # Handle the case where node pool already exists
  lifecycle {
    prevent_destroy = true
  }

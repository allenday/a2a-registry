...
class VectorGenerator:
    ...
    def __init__(self, model_name: str = "all-MiniLM-L6-v2"):
        ...
        logger.info(f"Initialized VectorGenerator with model {model_name} ({self.vector_dimensions} dimensions)")
    ...
    def generate_agent_vectors(self, agent_card: Dict[str, Any]) -> List[registry_pb2.Vector]:
        """
        ...
        if "name" in agent_card:
            vectors.append(self._create_vector(
                agent_id, "name", agent_card["name"]
            ))
        
        if "description" in agent_card:
            vectors.append(self._create_vector(
                agent_id, "description", agent_card["description"]
            ))
        ...
        for i, skill in enumerate(skills):
            ...
            if "name" in skill:
                ...
                vectors.append(self._create_vector(
                    agent_id, f"skills[{i}].name", skill["name"]
                ))
            ...
            if "tags" in skill and isinstance(skill["tags"], list):
                ...
                vectors.append(self._create_vector(
                    agent_id, f"skills[{i}].tags", tags_text
                ))
            ...
            if skill_texts:
                ...
                vectors.append(self._create_vector(
                    agent_id, f"skills[{i}]", combined_text
                ))
        ...
        for i, extension in enumerate(extensions):
            ...
            if "description" in extension:
                ...
                vectors.append(self._create_vector(
                    agent_id, f"extensions[{i}].description", extension["description"]
                ))
            ...
            if extension_texts:
                ...
                vectors.append(self._create_vector(
                    agent_id, f"extensions[{i}]", combined_text
                ))
    ...
    def calculate_similarity(self, vector1: registry_pb2.Vector, vector2: registry_pb2.Vector) -> float:
        """
    ...
    def _create_vector(self, agent_id: str, field_path: str, content: str) -> registry_pb2.Vector:
        """
